...
LinphoneChatRoom *linphone_core_create_client_group_chat_room (LinphoneCore *lc, const char *subject) {
	LinphoneChatRoom *chatRoom = L_INIT(ChatRoom);
	L_SET_CPP_PTR_FROM_C_OBJECT(chatRoom, L_GET_CPP_PTR_FROM_C_OBJECT(lc)->createClientGroupChatRoom(L_C_TO_STRING(subject)));
	return chatRoom;
}
